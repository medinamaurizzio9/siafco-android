...
fun SplashScreen(
    ...
    Surface(modifier = Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) {
        Column(
            ...
            BrandHeader(logoSize = 132.dp)
            Spacer(Modifier.height(32.dp))
            CircularProgressIndicator(Modifier.size(36.dp))
            Spacer(Modifier.height(16.dp))
            androidx.compose.material3.Text(text = androidx.compose.ui.res.stringResource(R.string.splash_loading), style = MaterialTheme.typography.bodyMedium)
        }
    }
}
